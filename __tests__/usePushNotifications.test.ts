...
describe('usePushNotifications Hook', () => {
  ...
  beforeAll(() => {
    ...
    registerForPushNotificationsAsync = module.registerForPushNotificationsAsync;
  });
  ...
  it('should handle errors during token retrieval', async () => {
    const Notifications = require('expo-notifications');
    Notifications.getExpoPushTokenAsync.mockRejectedValueOnce(new Error('Token error'));
  ...
  it('should handle location update errors gracefully', async () => {
    const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation();
    mockGetCurrentPositionAsync.mockRejectedValueOnce(new Error('Location error'));
  ...
  it('should cleanup on unmount', async () => {
    const { result, unmount } = renderHook(() => usePushNotifications('test-user-456'));
  ...
  it('should handle console.log for token', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Push token:', 'ExponentPushToken[test]');
  ...
  it('should handle console.warn for registration failure', async () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith('Push registration failed', expect.any(Error));
    consoleWarnSpy.mockRestore();
  });
  ...
  it('should handle Alert.alert error without crashing', async () => {
    ...
    mockGetExpoPushTokenAsync.mockRejectedValueOnce(new Error('No "projectId" found'));
  ...
  it('should call upsert with correct location payload structure', async () => {
    ...
    expect(payload.last_location_lon).toBe(-74.0060);
